...
# epipolar line for the test point.
#
# Arguments:
...
def main():
    ...
    for row in control_pts:
        ...
        row_a = [x_right * x_left, x_right * y_left, x_right, y_right * x_left, y_right * y_left, y_right, x_left,
                 y_left, 1]
        A.append(np.array(row_a).reshape((1, -1)))
    A = np.vstack(A)
    ...
    #draw right image
    cv2.line(right_img, (0,int(-epipolar_line[2]/epipolar_line[1])), (int(cb_cols)-1, int(-epipolar_line[2]/epipolar_line[1]-(cb_cols-1)*epipolar_line[0]/epipolar_line[1])), (0, 0, 255), 2)
